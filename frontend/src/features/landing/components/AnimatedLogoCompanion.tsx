...
export const AnimatedLogoCompanion: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // 3. Timeline de Scroll: despegue y desplazamiento al lateral izquierdo
    ...
    // Wordmark desaparece al hacer scroll
    tl.to(wordmark, { opacity: 0, y: 26, duration: 0.6 }, 0);

    // El cohete y alas se desplazan al lateral izquierdo
    tl.to(
      positioner,
      {
        x: () => (isDesktop() ? 44 : -window.innerWidth * 0.3),
        yPercent: -56,
    ...
    // Estado ignición para keyframe loop
    ...
    const handleScrollPhysics = () => {
      ...
      if (document.body.classList.contains('rocket-ignited')) {
        const tilt = gsap.utils.clamp(-9, 9, velocity * 0.35);
        gsap.to(rocket, {
          rotation: tilt,
        ...
        // Estiramiento reactivo de la llama
        const intensity = gsap.utils.clamp(0.88, 1.25, 1 + Math.abs(velocity) * 0.014);
        gsap.to(flameInner, {
          scaleY: intensity,
          duration: 0.4,
          ease: 'power2.out',
    ...
    <>
      <style>{`
        @keyframes flame-breathe {
          ...
          50% { transform: scaleY(1.15) scaleX(0.94); }
        }
        body.rocket-ignited #companion-flame-inner {
          animation: flame-breathe 1.5s ease-in-out infinite;
      ...
      {/* Capa del Logo — fija en viewport con pointer-events-none */}
      <div
        ref={containerRef}
