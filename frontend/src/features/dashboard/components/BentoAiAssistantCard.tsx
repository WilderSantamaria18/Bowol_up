...
export const BentoAiAssistantCard: React.FC = () => {
  ...
  return (
    <div className="rounded-2xl bg-white dark:bg-[#181920]/80 backdrop-blur-2xl p-6 border border-zinc-200/90 dark:border-white/[0.08] shadow-sm dark:shadow-[0_8px_32px_-4px_rgba(0,0,0,0.45)] relative overflow-hidden flex flex-col justify-between space-y-4">
      {/* Background ambient radial glow */}
      <div className="absolute -left-12 -bottom-12 w-44 h-44 bg-orange-500/10 dark:bg-orange-500/15 rounded-full blur-3xl pointer-events-none" />

      <div>
        {/* Header */}
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2.5">
            <div className="w-7 h-7 rounded-lg bg-orange-500/10 dark:bg-orange-500/20 border border-orange-500/20 dark:border-orange-500/30 flex items-center justify-center text-orange-600 dark:text-orange-400">
          ...
          <span className="px-2.5 py-1 rounded-md bg-orange-500/10 text-orange-600 dark:text-orange-400 border border-orange-500/20 text-xs font-semibold">
            Insight Estratégico
          </span>
        </div>

        {/* Insight content container */}
        <div className="mt-4 p-4 rounded-xl bg-zinc-50 dark:bg-[#1a1b22]/70 border border-zinc-200/80 dark:border-white/[0.04] space-y-3">
      ...
      <div className="pt-2 flex items-center gap-2.5">
        ...
        >
          ...
          <span>Profundizar análisis</span>
        </button>
        ...
        <button
          ...
          className={`p-2.5 rounded-xl border transition-colors ${
            isBookmarked
              ...
              : 'bg-zinc-100 hover:bg-zinc-200 dark:bg-[#292931]/60 dark:hover:bg-[#33343c] border-zinc-200/80 dark:border-white/[0.06] text-zinc-500 hover:text-zinc-950 dark:text-zinc-400 dark:hover:text-white'
          }`}
          title={isBookmarked ? 'Guardado en biblioteca estratégica' : 'Guardar síntesis'}
        >
          {isBookmarked ? <Check className="w-4 h-4 text-emerald-500 dark:text-emerald-400" /> : <Bookmark className="w-4 h-4" />}
        </button>
      </div>
    </div>
  );
